...
### Tests
- Added unit tests for `read_with_limit` (under limit, exactly at limit, over limit) and `atomic_write` (overwrites rather than appends). (809abb2)
...
Nightly builds now report `nightly.YYYYMMDD.SHORTSHA` from `--version` instead of the previous `0.0.0-nightly.YYYYMMDD.SHORTSHA`. Nightly artifact filenames follow the same format. (088fed0)
...
Also fixes several pre-existing issues uncovered by the new checks: invalid `GPL-3` SPDX identifier, pending tests not being skipped in CI, failing `actionlint` tests caused by unresolvable reusable workflow references, and `clippy` warnings in integration test helpers. (8f871f7)
...
- Update taiki-e/install-action action to v2.68.33 (originally merged in [#28](https://github.com/jonathanrainer/ghafmt/pull/28)) (96edec2)
